...
const BusinessAddress = () => {
    ...
    React.useMemo(()=>{
        const fetchBusinessTypes = async () =>{
           let request = await axios.get('https://malika-globalsetup.azurewebsites.net/api/v1/setup/residentialAreas')
                        ...
                        <form style={{ width: '70%' }} onSubmit={formik.handleSubmit}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'row', marginTop: '20px' }}>
                                <div style={{ display: 'flex', flexDirection: 'column', width: '100%' }}>
                                    ...
                                    <select id="residentialAreaId" name="residentialAreaId" value={formik.values.residentialAreaId} onChange={formik.handleChange} style={formik.touched.residentialAreaId && formik.errors.residentialAreaId ? { paddingLeft: "10px", marginTop: '-15px', height: '45px', border: '1px solid red', backgroundColor: 'white', fontSize: "16px" } : { paddingLeft: "10px", marginTop: '-15px', height: '45px', border: '1px solid gray', backgroundColor: 'white', fontSize: "16px", }} >
                                        <option value="" label="Select Residential Area"> </option>
                                        {data.map((item)=>(
                                             <option key={item.Id} value={item.Id}>{item.Name}</option>
                                        ))}
                                    </select>
                                    {/*{formik.values.residentialAreaId}*/}
